...
%% @doc  transaction given a mutation function.
-spec transaction(ref(), id(), actor()) -> {ok, var()} | {error, timeout}.
transaction(Ref, Id, Function) ->
	gen_server:call(Ref, {transaction, Id, Function}, infinity).
...
handle_call({transaction, Id, Function}, _From, #state{ref=Ref}=State) ->
    Result = case do_get(Ref, Id) of
        {ok, Value} ->
            {NewValue, InnerResult} = Function(Value),
            case do_put(Ref, Id, NewValue) of
