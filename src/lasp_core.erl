...
-spec transaction(list(), actor(), store()) -> 
		  {ok, var()} | not_found().
transaction(List, Actor, Store) ->
	MetadataFun = fun(X) -> X end,
    MetadataFunDeclare = fun(X) -> X end,
    transaction(List, Actor, MetadataFun, MetadataFunDeclare, Store).

-spec transaction(list(), actor(), function(), function(), store()) ->
    {ok, var()} | not_found().
transaction([Head|Tail], Actor, MetadataFun, MetadataFunDeclare, Store) ->
	{{_, Type}=Id, Operation} = Head,
	case do(get, [Store, Id]) of
		{ok, #dv{value=Value0, type=Type}} ->
            {ok, Value} = lasp_type:update(Type, Operation, {Id, Actor}, Value0),
			case Tail of 
				...
				_ -> 
					bind(Id, Value, MetadataFun, Store),
					transaction(Tail, Actor, MetadataFun, MetadataFunDeclare, Store)
			end;
        {error, not_found} ->
            {ok, _} = declare(Id, Type, MetadataFunDeclare, Store),
            update(Id, Operation, Actor, MetadataFun, MetadataFunDeclare, Store)
