...
blocking_sync(ObjectFilterFun) ->
	%Can be a list of update or a simple update
    ...
    case length(Peers) > 0 of
         true ->
            ...
            SyncFun = fun(Peer) ->
							...
							io:format(io_lib:format("=============> ~p's new buffer ...~n",[Peer]), []),
							...
							io:format(io_lib:format("=============> Ack received from : ~p~n",[Peer]), []),
							io:format(io_lib:format("=============> ~p's buffer deleted ...~n",[Peer]), []),
...
handle_info({state_sync, ObjectFilterFun},
            #state{store=Store, gossip_peers=GossipPeers, buffer=_Buffer_list} = State) ->
...
%% @private
send_buffered_update(Store, BufferId, Key, Index) ->
	case Index == Key of 
		true -> 
			{_, Function} = hd(ets:lookup(BufferId, Key)),
		...
		false -> 
			{_, Function} = hd(ets:lookup(BufferId, Key)),
		    %% TODO: Should this be parallel?
		    lasp_storage_backend:do(fold, [Store, Function, []]),
			send_buffered_update(Store, BufferId, Key, Index+1)
	end.
...
init_state_sync(Peer, ObjectFilterFun, Blocking, Store, BufferId, Key) ->
	case ObjectFilterFun of 
		...
		[Head_ObjectFilterFun|Tail] ->
		    ...
		    Function = fun({Id, #dv{type=Type, metadata=Metadata, value=Value}}, Acc0) ->
		                    case orddict:find(dynamic, Metadata) of
		                        {ok, true} ->
		                            ...
		                            Acc0;
		                        _ ->
		                            case Head_ObjectFilterFun(Id) of
		                                true ->
			...
			ets:insert(BufferId, {Key, Function}),
			case Tail of 
				[] -> 
					send_buffered_update(Store, BufferId, Key, 0);
				_ -> init_state_sync(Peer, Tail, Blocking, Store, BufferId, Key+1)
			end;
		%Or a simple ObjectFilterFun	
		_ -> 
		    % lasp_logger:extended("Initializing state propagation with peer: ~p", [Peer]),
		    Function = fun({Id, #dv{type=Type, metadata=Metadata, value=Value}}, Acc0) ->
		                    case orddict:find(dynamic, Metadata) of
