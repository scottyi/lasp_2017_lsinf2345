...
%Project : export transaction function
...
%% @doc Take a list of uptdate to perform
%%      
%%      The List provided would contain pairs of ObjectId and Operation. 
%%      The ActorId provided identify this actor uniquely in the system. 
%%      This call should take a list of updates to be performed 
%%      in the form of the parameters to the update call.
%%      ex: ([{ObjectId, Operation}], Actor)
-spec transaction(list(), actor()) -> {ok, var()} | {error, timeout}.
transaction(List, Actor) ->
	gen_server:call(?MODULE, {transaction, List, Actor}, infinity).
...
%% Perform an transaction, and ensure that we bump the logical clock as we
%% perform the trasaction.
%%
%% The CRDT actor is used to distinguish actors *per-thread*, if
%% necessary, where the vclock is serialized *per-node*.
%%
handle_call({transaction, List, CRDTActor}, _From,
            #state{store=_Store, actor=_Actor}=State) ->
	
	Result = result_transaction_list(List, CRDTActor, State, []),
...
%% @private
blocking_sync(Object, ObjectFilterFun_list) ->
	case Object of
		{Id, Metadata} -> 
		        ...
		        _ ->
					...
					case Tail of 
						[] -> 
							case lasp_config:get(mode, ?DEFAULT_MODE) of
				                state_based ->
				                    lasp_state_based_synchronization_backend:blocking_sync(lists:append(ObjectFilterFun_list, [ObjectFilterFun]));
				                delta_based ->
				                    {error, not_implemented}
				            end;
						_ -> blocking_sync(Tail, lists:append(ObjectFilterFun_list, [ObjectFilterFun]))
					end
		    end
	end.
...
%% @private
result_transaction_list(List, CRDTActor, #state{store=Store, actor=Actor}=State, Result_list)->
	case List of 
		[{Id, Operation}|Tail] -> 
			    lasp_marathon_simulations:log_message_queue_size("transaction"),
			
			    _Result = case lasp_config:get(intermediary_node_modification,
                                  ?INTERMEDIARY_NODE_MODIFICATION) of
			        true ->
			            Result0 = ?CORE:update(Id, Operation, CRDTActor, ?CLOCK_INCR(Actor),
			                                   ?CLOCK_INIT(Actor), Store),
						...
						case Tail of 
							[] ->
								%set blocking_sync to true
								%update will block waiting for an ACK 
								%from all neighbor nodes
							    lasp_config:set(blocking_sync, true),
								case lasp_config:get(blocking_sync, false) of
					                        true ->	
												io:format("=============> Waiting for ACKs !~n"),
					                            ok = blocking_sync(lists:append(Result_list, [{Id, Metadata}]), []);
					                        false ->
												io:format("=============> NO Waiting for ACKs !~n"),
					                            ok
					            end,
								Final;
							_ -> result_transaction_list(Tail, 
														 CRDTActor, 
														 State, 
														 lists:append(Result_list, [{Id, Metadata}]))
						end;
